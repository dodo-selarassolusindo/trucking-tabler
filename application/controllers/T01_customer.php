...
class T01_customer extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if ($q <> '') {
            $config['base_url'] = base_url() . 't01_customer?q=' . urlencode($q);
            $config['first_url'] = base_url() . 't01_customer?q=' . urlencode($q);
        } else {
            $config['base_url'] = base_url() . 't01_customer';
            $config['first_url'] = base_url() . 't01_customer';
        }

        $config['per_page'] = 10;
        $config['page_query_string'] = TRUE;
        $config['total_rows'] = $this->T01_customer_model->total_rows($q);
        $t01_customer = $this->T01_customer_model->get_limit_data($config['per_page'], $start, $q);

        $this->load->library('pagination');
        $this->pagination->initialize($config);

        $data = array(
            't01_customer_data' => $t01_customer,
            'q' => $q,
            'pagination' => $this->pagination->create_links(),
            'total_rows' => $config['total_rows'],
            'start' => $start,
        );
        // $this->load->view('t01_customer/t01_customer_list', $data);
...
/* Generated by Harviacode Codeigniter CRUD Generator 2024-01-31 10:25:48 */
/* http://harviacode.com */